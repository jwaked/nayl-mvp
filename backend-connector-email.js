import { truncate } from './backend-lib-text.js';
...
export function createManagedEmailConnector({ settingsService, appBaseUrl, timeoutMs }) {
  async function current() {
    const runtime = await settingsService.runtime();
    return createEmailConnector({
      apiKey: runtime.resend.apiKey,
      from: runtime.resend.emailFrom,
      appBaseUrl,
      timeoutMs
    });
  }
  ...
  return {
    async descriptor() {
      return (await current()).descriptor;
    },
    async send(message) {
      return (await current()).send(message);
    }
  };
}
