...
import { businessMatchesOpportunity } from './service-matching.js';
...
export class BusinessService {
  constructor({ store, sessionSecret, autoVerifyBusinesses, notification, appBaseUrl, adminEmail }) {
    this.store = store;
    this.sessionSecret = sessionSecret;
    this.autoVerifyBusinesses = autoVerifyBusinesses;
  ...
  async register(input) {
    ...
    const business = await this.store.transact((data) => {
      ...
      const created = {
        ...
        acceptingLeads: true,
        status: this.autoVerifyBusinesses ? 'verified' : 'pending',
        rating: null,
        reviewCount: 0,
    ...
    await Promise.allSettled([
      this.notification.send({
        ...
        heading: this.autoVerifyBusinesses ? 'Your NAYL profile is live' : 'Your profile is awaiting verification',
        body: this.autoVerifyBusinesses
